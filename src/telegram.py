...
class TelegramBot:
    ...
    async def send_error(self, err):
        msg = f"⚠️ Bot Error:\n<pre>{err}</pre>"
        logging.error(f"Telegram error: {err}") # Log the error
        await self._send(msg)
    ...
    async def _send(self, msg, retry=2):
        for i in range(retry):
            try:
                ...
                if i == retry - 1:
                    logging.error(f"Telegram send failed after {retry} retries: {e}")
                else:
                    logging.warning(f"Telegram send failed, retrying ({i+1}/{retry}): {e}")
